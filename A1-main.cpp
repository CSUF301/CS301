...
struct  PERSON 
	...
	void NewCopy(string inData, PERSON a[], int N);
	void Deposit(string customer, PERSON a[], int N);
...
void FindRichest(PERSON a[], int N)
{
	...
	for (int i = 1; i < N; i++)
	{
		if (a[i-1].Balance < a[i].Balance)
			slotrichest = i;	
	}
	cout << a[slotrichest].Name << endl;
}
